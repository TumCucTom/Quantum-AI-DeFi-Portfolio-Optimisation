...
export default function Dashboard() {
  ...
  const addPanel = (widgetType: string, widgetTitle: string) => {
    const uniqueId = `${widgetTitle}-${panels.length + 1}`;
  ...
  const handleSendPrompt = async (userPrompt: string) => {
    ...
    try {
      const res = await fetch("/api/execute", {
        ...
        body: JSON.stringify({ prompt: userPrompt, userApiKey: apiKey, active: activeEnhancements }),
      });
      const result = await res.json();
      const message = result.transactions?.map((t: any) => JSON.stringify(t)).join("\n\n") || JSON.stringify(result);
      setChatHistory((prev) => [...prev, { role: "assistant", text: message }]);
    } catch (error) {
      console.error("Error executing prompt: ", error);
      setChatHistory((prev) => [...prev, { role: "assistant", text: "Error executing prompt." }]);
    }
  };
  ...
  if (!isConfigured && showSettings) {
    return (
      <div className="min-h-screen bg-gradient-to-b from-[#000510] to-[#002240] text-white">
        <SettingsModal isOpen={true} onClose={() => setShowSettings(false)} onSave={handleSaveSettings} />
      </div>
    );
  }
    ...
    <div className="min-h-[calc(100vh-60px)] bg-gradient-to-b from-[#000510] to-[#002240] text-white">
      <div className="p-4 flex justify-between items-center border-b border-blue-400/20">
        ...
        <div className="flex gap-3">
          <button onClick={() => setShowSettings(true)} className="p-2 rounded-full bg-blue-900/30 border border-blue-400/30 hover:bg-blue-800/40 transition-colors" title="Dashboard Settings">
            <Settings size={20} className="text-blue-400" />
          </button>
          <button onClick={() => setShowWidgetMenu(!showWidgetMenu)} className="p-2 rounded-full bg-blue-900/30 border border-blue-400/30 hover:bg-blue-800/40 transition-colors" title="Add Widgets">
      ...
      <div className="flex lg:flex-row h-auto lg:h-[calc(100vh-124px)]">
        <main className="flex-grow p-4 overflow-auto">
          <div className="mb-6 px-4 py-3 bg-blue-900/20 border border-blue-400/20 rounded-lg flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
            ...
            <button onClick={() => setShowSettings(true)} className="text-xs text-blue-400 hover:text-blue-300 underline">Change</button>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6">
            <div className="relative bg-blue-900/50 border border-blue-400/30 rounded p-6 text-white min-h-[300px] max-h-[400px] overflow-y-auto">
              <p className="font-semibold text-lg mb-4 flex justify-between items-center">
                Control
                <button onClick={() => setShowInfoPopup(!showInfoPopup)} className="ml-2 text-blue-300 hover:text-blue-100" title="More information">
              ...
              {showInfoPopup && (
                <div ref={infoPopupRef} className="fixed top-1/2 left-1/2 w-[70rem] z-50 p-16 bg-blue-950 border border-blue-400/30 rounded text-base text-blue-100 shadow-2xl transform -translate-x-1/2 -translate-y-1/2">
                  <button onClick={() => setShowInfoPopup(false)} className="absolute top-3 left-4 text-red-400 hover:text-red-300 text-4xl font-bold" aria-label="Close">✕</button>
                  <ul className="text-xl list-disc list-inside space-y-5">
                    ...
                    <li><strong>QOS</strong>: Quantum-optimized slicing...</li>
                    <li><strong>QOR</strong>: Quantum-optimized route selection...</li>
                    <li><strong>QLCA</strong>: Quantum latency cost awareness...</li>
                  </ul>
                  <div className="mt-8 flex justify-center">
                    <button className="px-6 py-2 bg-blue-700 hover:bg-blue-600 text-white font-medium rounded" onClick={() => setShowInfoPopup(false)}>
                      Done
                    </button>
                  </div>
                </div>
            ...
            {panels.map((panel) => (
              <div key={panel.id} className="bg-white/5 border border-blue-400/20 rounded p-6 text-white min-h-[300px] max-h-[400px] overflow-y-auto">
                <div className="flex justify-between items-center mb-2">
                  <span className="text-base font-semibold capitalize">{panel.id}</span>
                  <button onClick={() => removePanel(panel.id)} className="text-sm text-red-400 hover:text-red-300">✕</button>
                </div>
      ...
      {showWidgetMenu && (
        <WidgetMenu onSelectWidget={(type, name) => addPanel(type, name)} onClose={() => setShowWidgetMenu(false)} />
      )}
